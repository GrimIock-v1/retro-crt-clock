...
<div class="stat"><span>Uptime</span><b id="uptime">—</b></div><div class="stat"><span>Weather</span><b id="weather">—</b></div><div class="stat"><span>Next event</span><b id="event">—</b></div>
</div><div class="buttons"><button id="refreshStatusBtn" type="button">Refresh status</button><button id="refreshDataBtn" type="button">Refresh data now</button><button id="diagBtn" type="button">Toggle diagnostics</button><button id="freezeBtn" type="button">Freeze display 30 sec</button><button id="restartBtn" class="danger" type="button">Restart clock</button></div><div class="msg" id="actionMsg" aria-live="polite"></div></section>
...
function formBody(displayOnly=false){const p=new URLSearchParams();const ids=displayOnly?['drift_pixels','drift_minutes','night_brightness','clock_brightness','background_brightness','city_speed','time_format','temperature_unit','weather_text','day_bar']:['bridge_url','timezone_rule','drift_pixels','drift_minutes','night_brightness','clock_brightness','background_brightness','city_speed','time_format','temperature_unit','weather_text','day_bar'];for(const id of ids){const el=$(id);p.set(id,el.type==='checkbox'?(el.checked?'1':'0'):el.value)}return p}
...
async function loadSettings(){try{const r=await fetch('/api/settings',{cache:'no-store'});const s=await r.json();$('bridge_url').value=s.bridge_url;$('timezone_rule').value=s.timezone_rule;$('drift_pixels').value=s.drift_pixels;$('drift_minutes').value=s.drift_minutes;$('night_brightness').value=s.night_brightness;$('clock_brightness').value=s.clock_brightness;$('background_brightness').value=s.background_brightness;$('city_speed').value=s.city_speed;$('time_format').value=s.use_24h?'24':'12';$('temperature_unit').value=s.fahrenheit?'F':'C';$('weather_text').checked=s.weather_text;$('day_bar').checked=s.day_bar;for(const id of ranges)$(id+'_v').textContent=$(id).value;$('version').textContent='Firmware '+s.firmware+(s.preview_active?' • preview active':'')}catch(e){msg('settingsMsg','Could not load settings: '+e.message)}}
...
$('diagBtn').addEventListener('click',async()=>{try{const j=await post('/api/diagnostics','');msg('actionMsg',j.message||'Diagnostics toggled')}catch(e){msg('actionMsg',e.message)}});$('freezeBtn').addEventListener('click',async()=>{try{const j=await post('/api/video-freeze','');msg('actionMsg',j.message||'Display frozen for 30 seconds')}catch(e){msg('actionMsg',e.message)}});
